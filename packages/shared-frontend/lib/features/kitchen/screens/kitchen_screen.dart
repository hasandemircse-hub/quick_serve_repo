  ...
  Future<void> _loadOrders() async {
    try {
      ...
      showCriticalFallbackSnackBar(
        context,
        actionLabel: 'Mutfak siparişleri yenileme',
        error: e,
        onRetry: _loadOrders,
      );
    }
  ...
  Future<void> _startPreparing(dynamic orderId) async {
    try {
      ...
      if (mounted) {
        showCriticalFallbackSnackBar(
          context,
          actionLabel: 'Hazırlama başlatma',
          error: e,
          onRetry: () => _startPreparing(orderId),
        );
      }
    }
  }
  ...
  Future<void> _markReady(dynamic orderId) async {
    try {
      ...
      if (mounted) {
        showCriticalFallbackSnackBar(
          context,
          actionLabel: 'Siparişi hazır yapma',
          error: e,
          onRetry: () => _markReady(orderId),
        );
      }
    }
  }
